...
## 예제1(my_launch.py)과의 핵심 차이점

| 기능 | 예제1 (my_launch.py) | C1 launch 파일 |
|---|---|---|
| **명령행 인자** | 없음 | `DeclareLaunchArgument`로 선언 |
| **파라미터 전달** | 없음 | `parameters=[{...}]`로 노드에 전달 |
| **실행 시 변경** | 불가 | `serial_port:=/dev/ttyUSB1` 등 가능 |
| **rviz2 연동** | 없음 | rviz2 노드 동시 실행 |
| **namespace** | 있음 (`launch_example1/2`) | 없음 |
| **노드 수** | 4개 (pub/sub × 2) | 2개 (sllidar_node + rviz2) |

---

## LaunchConfiguration vs DeclareLaunchArgument 관계

```
LaunchConfiguration('serial_port', default='/dev/ttyUSB0')
        ↓ 변수처럼 값을 참조하는 역할

DeclareLaunchArgument('serial_port', default_value=...)
        ↓ launch 시스템에 인자를 등록하는 역할

실행 시: ros2 launch sllidar_ros2 view_sllidar_c1_launch.py serial_port:=/dev/ttyUSB1
        ↓ 명령행 값이 default를 덮어씀

Node(parameters=[{'serial_port': serial_port}])
        ↓ 최종값이 노드 파라미터로 전달됨
```
...
# 포트 변경
ros2 launch sllidar_ros2 view_sllidar_c1_launch.py serial_port:=/dev/ttyUSB1

# 스캔 모드 변경
ros2 launch sllidar_ros2 view_sllidar_c1_launch.py scan_mode:=Standard

# 여러 인자 동시 변경
ros2 launch sllidar_ros2 view_sllidar_c1_launch.py serial_port:=/dev/ttyUSB1 scan_mode:=Standard
```
