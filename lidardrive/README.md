...
flowchart TD
    A(["/scan 토픽 수신"])
    B["② 스캔 영상 생성\n극좌표 → 픽셀 좌표 500×500, 반경 1.5m"]
    C["③ 장애물 마스크 생성\nBGR 0,0,200 ~ 50,50,255 빨간 픽셀 추출"]
    D["④ 전방 180도 분리\ny: 0 ~ CY  /  좌 x<CX · 우 x≥CX"]
    E["⑤ 좌/우 최단거리 장애물 검출\ndist = dx²+dy²  |  MIN_PIXELS ≥ 20"]
    F{⑥ 한쪽만\n장애물?}
    G["가상 장애물 배치\n없는 쪽 끝 x=0 / x=499"]
    H["⑦ error 각도 계산\natan2(mid_x-CX, CY-125) × 180/π"]
    I["⑧ 스무딩\nerror = prev × 0.9 + raw × 0.1"]
    J["⑨ P제어 속도 계산\nL = BASE+KP×err  /  R = -(BASE-KP×err)"]
    K(["/topic_dxlpub 발행 + mp4 저장"])
